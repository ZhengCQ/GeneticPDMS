...
import expandTable from '@/components/Table/urlExpandTable'
...
export default {
  ...
  methods: {
    getList() {
      ...
      console.log(this.$route.query.genename)
      this.listQuery.gene_name_list = this.$route.query.genename
    ...
    async handleDelete(row) {
      if (confirm('确定要删除吗？')) {
        ...
        } else if (row.gene_name) {
          this.deleted.gene_name = row.gene_name
        }
    ...
    resetDialog() {
      ...
      this.$refs.addEditForm.$refs.subelform.itemDataList = []
    ...
    handleUpdate(row) {
      ...
      for (var name in this.subFormInfo) {
        if (name === 'relate_phenotypeMIMnum') {
          var templst = []
          for (var each in this.dialogFormInfo[name]) {
            templst.push(each)
          }
          console.log(templst)
          this.subFormInfo[name] = templst
        } else {
